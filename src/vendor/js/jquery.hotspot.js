...
(function () {
  ...
  function _positionHotspots(options) {
    const imageWidth = $(`${options.mainselector} ${options.imageselector}`).prop('naturalWidth')
    const imageHeight = $(`${options.mainselector} ${options.imageselector}`).prop('naturalHeight')

    const bannerWidth = $(options.mainselector).width()
    const bannerHeight = $(options.mainselector).height()
    $(options.selector).each(function () {
      let xPos = $(this).attr('x')
      let yPos = $(this).attr('y')
  ...
  $.fn.hotSpot = function (options) {
    ...
    const _options = $.extend({}, $.fn.hotSpot.defaults, options)
    ...
    switch (_options.bindselector) {
      case 'click':
        this.find(_options.selector).bind('click', (e) => { _bindHotspots(e.currentTarget, _options) })
        this.find(_options.selector).addClass('hotspot-on-click')
        break
      case 'hover':
        this.find(_options.selector).hover((e) => { _bindHotspots(e.currentTarget, _options) })
        break
      default:
        break
    }
