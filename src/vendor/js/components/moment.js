...
;(function (global, factory) {
  ...
  function mergeConfigs(parentConfig, childConfig) {
    const res = extend({}, parentConfig); let prop
  ...
  function makeFormatFunction(format) {
    const array = format.match(formattingTokens); let i; let length
  ...
  function handleStrictParse(monthName, format, strict) {
    let i; let ii; let mom; const llc = monthName.toLocaleLowerCase()
  ...
  function handleStrictParse$1(weekdayName, format, strict) {
    let i; let ii; let mom; const llc = weekdayName.toLocaleLowerCase()
  ...
  function computeWeekdaysParse() {
    ...
    let i; let mom; let minp; let shortp; let longp
    for (i = 0; i < 7; i++) {
      // make the regex if we don't have it already
      mom = createUTC([2000, 1]).day(i)
  ...
  function chooseLocale(names) {
    let i = 0; let j; let next; let locale; let split
  ...
  function updateLocale(name, config) {
    if (config != null) {
      let locale; let tmpLocale; let parentConfig = baseConfig
      // MERGE
  ...
  function configFromArray(config) {
    let i; let date; const input = []; let currentDate; let expectedWeekday; let yearToUse
  ...
  function configFromISO(config) {
    ...
    let allowTime; let dateFormat; let timeFormat; let tzFormat
  ...
  function calculateOffset(obsOffset, militaryOffset, numOffset) {
    ...
    const m = hm % 100; const h = (hm - m) / 100
    return h * 60 + m
  }
 ...
 */
  ...
  function bubble() {
    ...
    const data = this._data
    let seconds, minutes, hours, years, monthsFromDays
