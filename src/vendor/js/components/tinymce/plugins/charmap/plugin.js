(function () {
  const charmap = (function () {
    ...
    const isArray$1 = global$1.isArray
    ...
    const charmapFilter = function (charmap) {
      return global$1.grep(charmap, (item) => isArray$1(item) && item.length === 2)
    }
    ...
    const extendCharMap = function (editor, charmap) {
      ...
      if (userCharMapAppend) {
        const userDefinedGroup = global$1.grep(charmap, (cg) => cg.name === UserDefined)
    ...
    const getCharMap$1 = function (editor) {
    ...
    const CharMap = { getCharMap: getCharMap$1 }
    ...
    const scan = function (group, pattern) {
      const matches = []
    ...
    const open = function (editor, charMap) {
      ...
      const makeTabs = function () {
        return map(charMap, (charGroup) => ({
          title: charGroup.name,
          items: makeGroupItems(),
        }))
      }
      const currentTab = charMap.length === 1 ? Cell(UserDefined) : Cell('All')
      const makePanel = function () {
        return {
          type: 'panel',
          items: makeGroupItems(),
        }
      }
      const makeTabPanel = function () {
        return {
          type: 'tabpanel',
          tabs: makeTabs(),
        }
      }
      const scanAndSet = function (dialogApi, pattern) {
      ...
      const updateFilter = last$1((dialogApi) => {
        const { pattern } = dialogApi.getData()
    ...
    const init = function (editor, all) {
