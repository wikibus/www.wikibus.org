(function () {
  const wordcount = (function () {
    ...
    const global$1 = tinymce.util.Tools.resolve('tinymce.dom.TreeWalker')
    ...
    const SETS$1 = SETS
    const { OTHER } = characterIndices
    const getType = function (char) {
      let j; let set; let type = OTHER
      const setsLength = SETS$1.length
    ...
    const classify = function (str) {
      ...
      return map(str.split(''), memoized)
    }

    const ci = characterIndices
    ...
    const EMPTY_STRING$1 = EMPTY_STRING
    const WHITESPACE$1 = WHITESPACE
    const PUNCTUATION$1 = PUNCTUATION
    ...
    const getCount = function (textContent) {
      return {
        words: getWords(textContent).length,
        characters: textContent.length,
        charactersNoSpace: textContent.replace(/\s/g, '').length,
      }
    }
    const getEditorWordcount = function (editor) {
      return getCount(getTextContent(editor))
    }
    const getSelectionWordcount = function (editor) {
    ...
    const fireWordCountUpdate = function (editor, wordCount) {
      editor.fire('wordCountUpdate', { wordCount })
    }

    const updateCount = function (editor) {
      const wordCount = getEditorWordcount(editor)
      fireWordCountUpdate(editor, wordCount)
    }
    const setup = function (editor) {
      const debouncedUpdate = global$2.debounce(() => updateCount(editor), 300)
      editor.on('init', () => {
        updateCount(editor)
